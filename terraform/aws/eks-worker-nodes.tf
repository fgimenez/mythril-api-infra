...
resource "aws_iam_role" "mythril-api-node" {
...
resource "aws_iam_role_policy_attachment" "mythril-api-node-AmazonEKS_CNI_Policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
  role       = "${aws_iam_role.mythril-api-node.name}"
}

resource "aws_iam_role_policy_attachment" "mythril-api-node-AmazonEC2ContainerRegistryReadOnly" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"
  role       = "${aws_iam_role.mythril-api-node.name}"
}

resource "aws_iam_instance_profile" "mythril-api-node" {
  name = "terraform-eks-mythril-api"
  role = "${aws_iam_role.mythril-api-node.name}"
}

resource "aws_security_group" "mythril-api-node" {
  name        = "terraform-eks-mythril-api-node"
...
resource "aws_security_group_rule" "mythril-api-node-ingress-self" {
...
resource "aws_security_group_rule" "mythril-api-node-ingress-cluster" {
...
locals {
  mythril-api-node-userdata = <<USERDATA
#!/bin/bash -xe

CA_CERTIFICATE_DIRECTORY=/etc/kubernetes/pki
CA_CERTIFICATE_FILE_PATH=$CA_CERTIFICATE_DIRECTORY/ca.crt
mkdir -p $CA_CERTIFICATE_DIRECTORY
echo "${aws_eks_cluster.mythril-api.certificate_authority.0.data}" | base64 -d >  $CA_CERTIFICATE_FILE_PATH
INTERNAL_IP=$(curl -s http://169.254.169.254/latest/meta-data/local-ipv4)
sed -i s,MASTER_ENDPOINT,${aws_eks_cluster.mythril-api.endpoint},g /var/lib/kubelet/kubeconfig
sed -i s,CLUSTER_NAME,${var.cluster-name},g /var/lib/kubelet/kubeconfig
sed -i s,REGION,${data.aws_region.current.name},g /etc/systemd/system/kubelet.service
sed -i s,MAX_PODS,20,g /etc/systemd/system/kubelet.service
sed -i s,MASTER_ENDPOINT,${aws_eks_cluster.mythril-api.endpoint},g /etc/systemd/system/kubelet.service
...
resource "aws_autoscaling_group" "mythril-api" {
  ...
  tag {
    key                 = "Name"
    value               = "terraform-eks-mythril-api"
